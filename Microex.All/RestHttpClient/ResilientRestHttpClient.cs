...
{
    public class ResilientRestHttpClient:IRestHttpClient
    {
        private readonly HttpClient _client;
        private readonly ILogger<ResilientRestHttpClient> _logger;
        private readonly Func<string, IEnumerable<Policy>> _policyCreator;
        private ConcurrentDictionary<string, PolicyWrap> _policyWrappers;

        public ResilientRestHttpClient(ILogger<ResilientRestHttpClient> logger, ResilientPolicyOptions options, Func<string, IEnumerable<Policy>> policyCreator = null)
        {
            _client = new HttpClient();
            _logger = logger;
            _policyCreator = policyCreator ?? ((origin) => new Policy[]
            {
                Policy.Handle<HttpRequestException>()
                    .WaitAndRetry(
                        ...
                        curAttemptCount => TimeSpan.FromSeconds(Math.Pow(2, curAttemptCount)),
                        // on retry
                        (exception, timeSpan, retryCount, context) =>
                        {
                            var msg = $"Retry {retryCount} times " +
                ...
                Policy.Handle<HttpRequestException>()
                    .CircuitBreaker(
                        ...
                        options.CircuitBreakerExceptionCount,
        ...
        private Task<HttpResponseMessage> DoPostPutAsync<T>(HttpMethod method, string uri, T item, string authorizationToken = null, string requestId = null, string authorizationMethod = "Bearer")
        {
            ...
            return HttpInvoker(origin, context =>
            {
